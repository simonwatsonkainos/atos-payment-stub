 ...
 */
$.fn.generateSplit = function(selectedPaymentMean, paymentMeans) {
	var pixelsPerCharacter = 8;
	$('input#cardNumberField').hide(0);
	var input = "";
	var concatenate = "";

	var tab = [19];

	// One payment mean has already been chosen by user
	if ("" != selectedPaymentMean) {

		if (("CB" == selectedPaymentMean) || ("VISA" == selectedPaymentMean) || ("MASTERCARD" == selectedPaymentMean)) {
			tab = [4,4,4,7];
		}
		else if ("AMEX" == selectedPaymentMean) {
			tab = [4,6,5];
		}
	}
	// Check which payment mean could be chosen by user
	else {

		if (((paymentMeans.indexOf("CB") != -1) || (paymentMeans.indexOf("VISA") != -1)
				|| (paymentMeans.indexOf("MASTERCARD") != -1)) && (paymentMeans.indexOf("AMEX") == -1)) {
			tab = [4,4,4,7];
		}
		else if ((paymentMeans.indexOf("AMEX") != -1) && ((paymentMeans.indexOf("CB") == -1) && (paymentMeans.indexOf("VISA") == -1) && (paymentMeans.indexOf("MASTERCARD") == -1))) {
			tab = [4,6,5];
		}
		else{
			$('input#cardNumberField').show(0);
			return;
		}
	}
	...
	for (i = 0; i < length; i++)
	{
		$('#splitCardNumberBlock').append('<input type="text" id="cardNumberField'+i+'" maxlength="'+tab[i]+'" autocomplete="OFF" style="width:'+(tab[i]*pixelsPerCharacter)+'px;"/>');
		if (i<length-1)
			$('#splitCardNumberBlock').append(' - ');

		input = input + '#cardNumberField'+i+',';
		if (i < length-1)
			input = input +', ';

		// concatenate card number in cardNumberField when input changes
		$('#cardNumberField'+i+'').change(function(){
			var concatenate = "";
			for (i = 0; i<length; i++){
				concatenate = concatenate + $('#cardNumberField'+i+'').val();
				$('#cardNumberField').val(concatenate);
			}
		});
	}
	$(input).autotab_magic();
